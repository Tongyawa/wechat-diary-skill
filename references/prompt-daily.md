...
3. **读取 processed exports**：用 Glob 工具列出 `WeFlow-processed-exports/*/{yesterday}.md`。
   ...
   - **占位符（引用 / 媒体 / 表情 / 文件 / 链接等）的含义与处理方式，读 [processed-format.md](processed-format.md)**——那是唯一事实源，本文不复述（复述必然过时）。

   `朋友圈_自己/{yesterday}.md` 是例外输入：它不是聊天流，而是自己的朋友圈流，格式见 [processed-format.md](processed-format.md) §4。历史产物中旧式 `WeFlow-raw-exports/...` 绝对语义路径若已失效，按归档库 `WeFlow-archived-exports/raw/` 下的对应路径找。
