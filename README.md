...
日常归档仍按当前会话显示名建目录。若检查发现同一 wxid 已落在多个归档目录，收尾会只在首次发现或目录集合变化时提示；明细写在工作区根的 `.session-rename-state.json`，不进 git。目录文本不足以可靠判断它是改名还是人工变体，因此所有分裂都需要人工核对；检查不会自动移动或合并任何历史目录。
