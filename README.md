...
日常归档仍按当前会话显示名建目录。若发现同一 wxid 已落在多个归档目录，收尾会把旧目录逐文件并到**本轮实际导出的 displayName**目录；同路径异内容严格复用历史摄取的碰撞护栏，拒绝项保留原处。每次合并或停留都会写工作区根的 `.session-rename-report.md` 并尝试用 Notepad 打开，必须由用户手动关闭；本轮未导出的会话因没有可信新名而只留在报告中。状态仍写在 `.session-rename-state.json`，两者均不进 git，且不会改变导出退出码。
