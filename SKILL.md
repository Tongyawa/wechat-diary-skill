...
- **工作区** = 含 `config.toml` 的目录。所有 `WeFlow-*` 数据路径都从该配置解析；相对路径相对于工作区，**绝不相对于本 skill 目录**。当前目录没有 `config.toml` 时，先在用户给出的项目目录里找，仍无法确定再问。
...
| 备份本地 git 仓 | ⑦ bundle 备份 | `references/entrypoints.md` §7 |
...
**⑦ bundle 备份** `powershell ... -File "$SkillRoot\scripts\Backup-PrivateRepo.ps1" -RepoPath <仓> -Destination <落点>`
